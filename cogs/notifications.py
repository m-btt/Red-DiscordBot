...
class Notifications:
    ...
    @commands.group(pass_context=True, no_pm=True, name="notifications", aliases=["notification", "noti", "bark", "barks"])
    ...
    async def _add(self, ctx, *, keyword):
        ...
        for keywordData in self.keywords[server.id]:
            if keywordData["userId"] == author.id and keywordData["keyword"] == keyword:
                ...
                await self.bot.say("*bark bark* {0}, check your DMs for a list of barks! <:zero:431489909161852929>".format(author.name))
                await self.bot.send_message(author, "*grrr*... `{1}` is already making me bark at you on the server `{2.name}`! :feet:".format(author.mention, keyword, server))
                return
        ...
        await self.bot.delete_message(message)
        await self.bot.say("*bark* okay {0}! Please check your DMs! <:zeroblush:432995389851959296>".format(author.name))
        await self.bot.send_message(author, "I will bark at you whenever `{1}` is said on the `{2.name}` server! <:zeroblush:432995389851959296>".format(author.mention, keyword, server))

    @_notifications.command(pass_context=True, no_pm=True, name="remove", aliases=["del", "delete", "erase", "clear"])
    async def _remove(self, ctx, *, keyword):
        ...
        if server.id not in self.keywords:
            await self.bot.delete_message(message)
            await self.bot.say("*bork* okay {0}... Please check your DMs. <:zero:431489909161852929>".format(author.name))
            await self.bot.send_message(author, "*grrr* I can't find `{1}` in my list of barks for you on the `{2.name}` server! :feet:".format(author.mention, keyword, server))
            return
        ...
        for keywordData in self.keywords[server.id]:
            if keywordData["userId"] == author.id and keywordData["keyword"] == keyword:
                ...
                await self.bot.delete_message(message)
                await self.bot.say("*bork* alright {0}... Please check your DMs!".format(author.name))
                await self.bot.send_message(author, "*grrr* I will stop barking at you for `{1}` on the `{2.name}` server... :speak_no_evil:".format(author.mention, keyword, server))
                return

        await self.bot.delete_message(message)
        await self.bot.say("*bork* okay {0}... Please check your DMs. :zero:".format(author.name))
        await self.bot.send_message(author, "*grrr* I can't find `{1}` in my list of barks for you on the `{2.name}` server! :feet:".format(author.mention, keyword, server))
    ...
    async def _list(self, ctx):
        ...
        if server.id not in self.keywords:
            await self.bot.say("*grrr* You have no barks listed... :feet:")
            return
        ...
        keywordsMessage = "*yap* I'm going to bark at you for ALL of these things! Look: "
        i = 0
        ...
        await self.bot.say("{0}! *bark* Please check your DMs!".format(author.name))
    ...
    async def check_keyword(self, message):
        ...
        if len(toNotifyUserForList) > 0:
            ...
            for user in users:
                notifyMessage = "*bark!* **{0.author.name} ({0.author.mention}) mentioned {1} in `{0.server.name}`! {0.channel.mention}:** {0.content}".format(message, keywordListText)
                await self.bot.send_message(user, notifyMessage)

    def _words_in_text(self, haystack, needle):
        regex = r"((?:\b(?:" + re.escape(needle) + r"))[^a-z]([ @:_\-\"\,!\+.~?&//=]?){1})"
        if haystack == needle:
            return True
